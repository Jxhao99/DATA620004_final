...
def get_map(MINOVERLAP, draw_plot,count,writer, path = './map_out'):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            if len(prec)>0:
                ...
                recalll+=rec[score05_idx]
                pree+=prec[score05_idx]
                mlou+= float(tp[score05_idx])/(np.maximum(gt_counter_per_class[class_name], 1)+np.maximum((fp[score05_idx] + tp[score05_idx]), 1)-float(tp[score05_idx]))
            else:
        ...
        results_file.write(text + "\n")
        print(text)
        print("mlou="+str(mlou/n_classes))

        writer.add_scalar('Test/mAP', mAP, count)
        writer.add_scalar('Test/MloU', mlou/n_classes, count)
        writer.add_scalar('Test/precision', pree/n_classes, count)
        writer.add_scalar('Test/recall', recalll/n_classes, count)

        print(mlou/n_classes,pree/n_classes,recalll/n_classes)
