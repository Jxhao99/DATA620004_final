...
parser.add_argument('--train_gpu', default=[0,], type=list, help='gpu will be used')
parser.add_argument('--model_path', default='', type=str, help='model_path')
parser.add_argument('--pretrained', default=True, type=bool, help='pretrained or not')
parser.add_argument('--pre_data', default="imagenet", type=str, help='imagenet/coco')
parser.add_argument('--backbone', default="resnet50", type=str, help='resnet50/vgg16')
parser.add_argument('--Freeze_Epoch', default=0, type=int)
parser.add_argument('--Freeze_batch_size', default=4, type=int)
parser.add_argument('--UnFreeze_Epoch', default=100, type=int)
parser.add_argument('--Unfreeze_batch_size', default=8, type=int)
parser.add_argument('--Init_lr', default=1e-4, type=float)
parser.add_argument('--Min_lr', default=1e-6, type=float)
parser.add_argument('--Freeze_Train', default=False, type=bool)
parser.add_argument('--optimizer_type', default="adam", type=str)
parser.add_argument('--momentum', default=0.9, type=float)
parser.add_argument('--weight_decay', default=0, type=int)
parser.add_argument('--lr_decay_type', default="cos", type=str)
...
if __name__ == "__main__":
    ...
    #   resnet50
    #---------------------------------------------#
    backbone        = args.backbone #"resnet50"
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    Init_Epoch          = 0
    Freeze_Epoch        = args.Freeze_Epoch #50
    Freeze_batch_size   = args.Freeze_batch_size #4
    #------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = args.UnFreeze_Epoch #100
    Unfreeze_batch_size = args.Unfreeze_batch_size #2
    #------------------------------------------------------------------#
    #   Freeze_Train    是否进行冻结训练
    #                   默认先冻结主干训练后解冻训练。
    #                   如果设置Freeze_Train=False，建议使用优化器为sgd
    #------------------------------------------------------------------#
    Freeze_Train        = False #args.Freeze_Train #True
    ...
    Min_lr              = args.Min_lr #Init_lr * 0.01
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    optimizer_type      = args.optimizer_type #"adam"
    momentum            = args.momentum  #0.9
    weight_decay        = args.weight_decay #0
    #------------------------------------------------------------------#
    #   lr_decay_type   使用到的学习率下降方式，可选的有'step'、'cos'
    #------------------------------------------------------------------#
    lr_decay_type       = args.lr_decay_type #'cos'
    #------------------------------------------------------------------#
    ...
    model = FasterRCNN(num_classes, anchor_scales = anchors_size, backbone = backbone, pretrained = pretrained,pre_data = args.pre_data)
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    for file in os.listdir("runs/"):
        file = os.path.join("runs/", file)
        if os.path.isdir(file): #判断文件是否可以被打开
            file_count = file_count+1

    writer = SummaryWriter("runs/exp"+str(file_count+1))
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            #fit_one_epoch(model, train_util, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir,writer)
            
            torch.cuda.empty_cache()
            get_mAP(epoch,"logs/curr_weights.pth",writer)
